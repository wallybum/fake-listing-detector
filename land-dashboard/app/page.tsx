...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    statsQuery = statsQuery
      .gte("crawl_time", startTimeFull)
      .lte("crawl_time", endTimeFull);
    logsQuery = logsQuery
      .gte("crawl_time", startTimeFull)
      .lte("crawl_time", endTimeFull);
    ...
    const datasets = agents.map((agent, index) => {
      ...
      const dataPoints = uniqueTimePoints.map((label) => {
        const found = agentData.find((d) => {
          const timeStr = d.crawl_time.substring(0, 5);
          const currentLabel = isSingleDay
            ? timeStr
            : `${d.crawl_date.slice(5)} ${timeStr}`;
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 mb-6">
          {/* [수정 1] 높이: 모바일 auto / 데스크탑 500px */}
          {/* [수정 2] 배치: 기본 세로(col) / 큰 화면 가로(row) */}
          <div className="w-full h-auto lg:h-[500px] flex flex-col lg:flex-row gap-6 lg:gap-4">
            {/* 1. 차트 영역 */}
            {/* 모바일에서는 높이 300px 고정, 큰 화면에서는 부모 높이(100%) 따라감 */}
            <div className="flex-1 h-[300px] lg:h-full min-h-[300px]">
              {loading ? (
            ...
            {/* 2. 범례(목록) 영역 */}
            {/* 모바일: 너비 꽉 채움 / 데스크탑: 너비 280px 고정 */}
            {/* 모바일: 위쪽 테두리 / 데스크탑: 왼쪽 테두리 */}
            {!loading && chartData && (
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                  <ul className="space-y-1">
                    {chartData.datasets.map((dataset: any) => (
                      <li
                        key={dataset.label}
                        className="flex items-center gap-2 text-xs text-gray-600 hover:bg-gray-50 p-2 rounded cursor-default border border-transparent hover:border-gray-100 transition-colors"
                      >
                        {/* 색상 표시 원 */}
                        <span
