...
// ... (인터페이스 TimelineItem, AnalyzedListing은 기존과 동일) ...
...
export default function ListingLifecycleAnalysis({}: Props) {
  ...
  // ... (useEffect, handleDateChange, fetchLogs, toggleExpand는 기존과 동일) ...
  useEffect(() => {
  ...
  const handleDateChange = (type: "start" | "end", newValue: string) => {
    ...
    if (newEnd > oneMonthLimit) { alert("최대 1개월 기간까지만 조회할 수 있습니다.\n기간을 좁혀주세요."); return; }
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      let timeQuery = supabase.from("real_estate_logs").select("crawl_date, crawl_time")
        .gte("crawl_date", localStartDate).lte("crawl_date", localEndDate)
      ...
      if (term.length > 0) { if (timeResult.data) setAllTimeLogs(timeResult.data); }
      else { if (logsResult.data) setAllTimeLogs(logsResult.data); }
  ...
  const analyzedData = useMemo(() => {
    ...
    const analyzed: AnalyzedListing[] = Object.keys(groups).map((key) => {
      ...
      return {
        ...
        is_owner: !!(lastItem as any).is_owner, verification_date: (lastItem as any).verification_date || null,
        ...
        status, display_timeline: validTimeline, provider: lastItem.provider || "알수없음",
      };
    });
  ...
  // ▼▼▼ [수정 1] 카운트 로직 개선: Provider 필터 반영 ▼▼▼
  // 기존에는 전체 데이터 기준이었으나, 이제 선택된 CP(filterProvider)에 해당하는 데이터만 카운트합니다.
  ...
  }, [analyzedData, filterProvider]); // 의존성에 filterProvider 추가

  // ▼▼▼ [수정 2] 소유자 카운트 개선: Provider 필터 반영 ▼▼▼
  const ownerCounts = useMemo(() => {
    ...
    // Analysis Tab (counts 객체 재사용)
    return { 
        total: counts.analysisTotal, 
        owner: counts.analysisOwner, 
        agent: counts.analysisAgent 
    };
  }, [mainTab, analyzedData, filterProvider, counts]); // 의존성 추가

  // ... (listContent 렌더링 부분 기존과 동일) ...
  let listContent;
  if (loading) {
    listContent = <div className="flex flex-col items-center justify-center h-full text-gray-400"><Loader2 className="w-8 h-8 animate-spin mb-2" /><p>데이터를 불러오는 중입니다...</p></div>;
                                ...
                                return (
                                    <div key={idx} className="relative pl-8 pb-3 last:pb-0 flex items-start gap-3 group/item">
                                        <div className={`absolute left-0 w-10 h-10 flex items-start justify-center z-10`}>
                                            {log.status === "collected" ? <div className={`w-2.5 h-2.5 rounded-full mt-1.5 ring-4 ring-white ${isPriceChanged ? "bg-purple-500" : "bg-blue-500"}`}></div> : <div className="w-2.5 h-2.5 rounded-full bg-orange-400 mt-1.5 ring-4 ring-white"></div>}
                                        </div>
                                        <div className="flex-1">
                                            <div className="flex flex-wrap items-center gap-2 mb-1">
                                                ...
                                                {log.status === "collected" ? <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold border ${isPriceChanged ? "bg-purple-100 text-purple-700 border-purple-200" : "bg-white text-gray-600 border-gray-200"}`}>{isPriceChanged ? "⚡ 가격변경" : "수집됨"}</span> : <span className="text-[10px] bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded font-bold border border-orange-200 flex items-center gap-1"><MinusCircle className="w-3 h-3" /> 수집 안됨</span>}
                                            </div>
                                            {log.status === "collected" && <div className={`text-xs border p-2 rounded shadow-sm flex justify-between items-center bg-white ${isPriceChanged ? "border-purple-300" : "border-gray-200"}`}><span className="font-bold">{log.price}</span><span className="text-[10px] opacity-70">{log.agent}</span></div>}
                                        </div>
                                    </div>
                                );
                            })}
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-[700px]">
      ...
      {/* ▼▼▼ [수정 3] 삭제된 매물 탭에서도 엑셀 다운로드 가능하게 추가 ▼▼▼ */}
      {mainTab === "deleted" && (
        <div className="px-4 py-3 bg-gray-100 border-b border-gray-200 flex items-center justify-between">
            <span className="text-xs text-gray-500 font-bold">삭제된 매물 목록입니다.</span>
            <div className="flex items-center gap-2">
                {filterProvider !== "all" && (
